...
function renderVerdict(m) {
  ...
  reason.textContent = issues.length ? issues.join(" · ") : "All three pillars clear: valuation, financing and covenants hold.";
}
...
function renderKpis(m) {
  ...
  $("kpiNpvSub").textContent = npv >= 0 ? `PV ${fmt.money(m.valuation.pvCashFlows, 0)} − CAPEX` : `PV ${fmt.money(m.valuation.pvCashFlows, 0)} vs CAPEX ${fmt.money(inp.capex, 0)}`;

  const irrEl = $("kpiIrr");
  irrEl.textContent = fmt.pct(irr);
  irrEl.className = "kpi-value nums" + cls(irr, irr >= wacc);
  $("kpiIrrSub").textContent = "vs WACC " + fmt.pct(wacc);
  ...
  $("kpiLcoeSub").textContent = "vs price " + fmt.num(price, 0);

  $("kpiDebt").textContent = fmt.money(m.debt.debt, 0);
  $("kpiDebtSub").textContent = `Equity ${fmt.money(m.debt.equity, 0)} · ${fmt.pct(m.debt.actualGearing, 1)} gearing`;
  ...
  $("kpiDscrSub").textContent = "min " + fmt.mult(inp.minDscr) + ` · ${m.debt.binding}-bound`;
  ...
  $("kpiEquityIrrSub").textContent = "vs " + fmt.pct(inp.re);
}
...
function renderRisk(m) {
  ...
  // Burial — reference figures (per study case): 1m = 2.0% / $0.30m pa loss,
  // 3m = 0.5% / $0.075m pa loss, +$15m CAPEX for 3m.
  const depth = parseInt($("selBurial").value, 10);
  const show3 = depth === 3;
  const fail = show3 ? 0.005 : 0.02;
  const loss1 = 0.300, loss3 = 0.075;
  const yloss = show3 ? loss3 : loss1;
  const pvLoss = yloss * m.valuation.afWacc;
  const pvSave = (loss1 - loss3) * m.valuation.afWacc; // PV of switching 1m -> 3m
  kv([
    ["Burial depth", depth + " m (DoL)"],
    ["Annual fault probability", fmt.pct(fail, 1)],
    ["Annual expected loss", fmt.money(yloss, 3)],
    ["PV of loss over life", fmt.money(pvLoss, 1)],
    ["vs 1m baseline saving (3m)", show3 ? fmt.money(pvSave, 1) : "—"],
    ["Added CAPEX (3m)", show3 ? "—" : `+${fmt.money(15, 0)} (vs ${fmt.money(pvSave, 1)} save)`],
  ], "burialGrid");
  const burialNote = $("burialGrid");
  burialNote.insertAdjacentHTML("beforeend",
    `<div class="kv"><span class="k">${show3 ? "Trade-off" : "Surface NPV says"}</span><span class="v warn">` +
    (show3
      ? "NPV says not worth; bankability flips it (insurability + covenant breach)"
      : "3m adds +$15m but cuts fault risk 4× — insurance & DSCR lock-up argue the case") +
    `</span></div>`);
}
...
function bind() {
  const ids = Object.values(INPUT_IDS).concat(["tglIncome", "tglIndex", "tglVessel", "tglOm", "selBurial"]);
  ids.forEach((id) => {
    const el = $(id);
    if (!el) return;
